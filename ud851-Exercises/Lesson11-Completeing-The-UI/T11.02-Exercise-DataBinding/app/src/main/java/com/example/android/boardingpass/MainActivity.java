...
public class MainActivity extends AppCompatActivity {
    ...
    private void displayBoardingPassInfo(BoardingPassInfo info) {
        ...
        String arivalTime=formatter.format(info.arrivalTime);

        mBinding.textViewBoardingTime.setText(boardingTime);
        mBinding.textViewDepartureTime.setText(depatureTime);
        mBinding.textViewArrivalTime.setText(arivalTime);

        // TODO (8) Use TimeUnit methods to format the total minutes until boarding
        long totalminites=info.getMinutesUntilBoarding();
        long totalhours= TimeUnit.MINUTES.toHours(totalminites);
        long minteslessthan=totalminites-TimeUnit.HOURS.toMinutes(totalhours);
        String timeHoursaandMinites=getString(R.string.countDownFormat,totalhours,minteslessthan);
        mBinding.textViewBoardingInCountdown.setText(timeHoursaandMinites);
