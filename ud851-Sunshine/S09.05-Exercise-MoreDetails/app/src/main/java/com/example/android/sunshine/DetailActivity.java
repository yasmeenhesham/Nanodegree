...
//  TODO (20) Create a constant int to identify our loader used in DetailActivity
    private static final int ID_DETAIL_LOADER = 77;
...
//      TODO (12) Remove mWeatherDisplay TextView
        ...
        daydescription =(TextView) findViewById(R.id.daydescription);
        hightemp=(TextView) findViewById(R.id.hightemp) ;
        lowtemp=(TextView)findViewById(R.id.lowtemp);
        dayhumidity=(TextView)findViewById(R.id.dayhumidity);
        daypressure=(TextView)findViewById(R.id.daypressure);
        daywind=(TextView)findViewById(R.id.daywind) ;
...
//  TODO (24) Override onLoadFinished
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if(!dataflag)
            return;
        long returndata=data.getLong(INDEX_WEATHER_DATE);
        String stringdata= SunshineDateUtils.getFriendlyDateString(this,returndata,false);
        daydate.setText(stringdata);

        int wetherID= data.getInt(INDEX_WEATHER_CONDITION_ID);
        String des=SunshineWeatherUtils.getStringForWeatherCondition(this,wetherID);
        daydescription.setText(des);
        double high=data.getDouble(INDEX_WEATHER_MAX_TEMP);
        String highStr=SunshineWeatherUtils.formatTemperature(this,high);
        hightemp.setText(highStr);
        double low=data.getDouble(INDEX_WEATHER_MIN_TEMP);
        String lowStr=SunshineWeatherUtils.formatTemperature(this,low);
        lowtemp.setText(lowStr);
        float hum=data.getFloat(INDEX_WEATHER_HUMIDITY);
        String humStr=getString(R.string.format_humidity,hum);
        dayhumidity.setText(humStr);
        float wind=data.getFloat(INDEX_WEATHER_WIND_SPEED);
        float dir=data.getFloat(INDEX_WEATHER_DEGREES);
        String windDir=SunshineWeatherUtils.getFormattedWind(this,wind,dir);
        daywind.setText(windDir);
        float press=data.getFloat(INDEX_WEATHER_PRESSURE);
        String pressStr=getString(R.string.format_pressure,press);
        daypressure.setText(pressStr);
        mForecastSummary=String.format("%s - %s - %s/%s",stringdata,des,highStr,lowStr);
    }
